...
def simple(g):
	...
	# YOUR CODE HERE
	edges_old = g.edges
	edges_new = edges_old.select(edges_old.dst.alias('src'), edges_old.src.alias('dst'))
	...
	# YOUR CODE HERE
	edges_all = edges_old.unionAll(edges_new).distinct()
	edges_all = edges_all.filter(edges_all.src != edges_all.dst)
	return GraphFrame(g.vertices, edges_all)
...
if len(sys.argv) > 1:
	...
	distribution.toPandas().to_csv(out + ".csv")
...
# Otherwise, generate some random graphs.
	...
	for gx in todo:
		...
		distribution = degreedist(g)
		print("Writing distribution to file " + gx + ".csv")
		distribution.toPandas().to_csv(gx + ".csv")
